...
Trader.prototype.createOrder = function(side, amount, advice, id) {
  ...
  this.order.on('completed', () => {
    this.order.createSummary((err, summary) => {
      ...
      // CUBIT 161018 - here want to catch this date - 1970-01-01T00:00:00.000 - and...
      ...
      // CUBIT 211018 - no hits so far! Amended to use === rather than ==
      // CUBIT 211018 - Also... looking at the structure ( date: moment("1970-01-01T00:00:00.000"), ) this is a real long shot but going to try as follows...
      // CUBIT 210118 - under test on 3001, 3014, 3016, 3018
      if (summary.date==='1970-01-01T00:00:00.000' || summary.date==='moment("1970-01-01T00:00:00.000")' || summary.date===moment('1970-01-01T00:00:00.000')) {
        summary.date=new Date().toISOString().replace(/Z/,'');
        // and log what we've done to the console so I can see if it's worked! :)
        log.info('[CUBIT] amended null date to :', summary.date);
      }
